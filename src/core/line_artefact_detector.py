...
# A column/row is a line candidate when BOTH hold:
...
#Confidence scoring
CONFIDENCE_WIDTH_REF = 30
CONFIDENCE_WIDTH_POWER = 0.45
CONFIDENCE_STEEPNESS = 6.0
CONFIDENCE_RANK_DECAY = 0.50


def _load_gray_from_array(arr: NDArray) -> NDArray[np.float32]:
    """
...
def _detect_axis(
    ...
    """
    Detect image lines along one axis and return a list of line descriptors.
    ...
    Returns:
        List of dicts, one per detected image line, containing type, centre,
...
def compute_confidence(
        lines: list[dict],
        width_ref: float = CONFIDENCE_WIDTH_REF,
        width_power: float = CONFIDENCE_WIDTH_POWER,
        steepness: float = CONFIDENCE_STEEPNESS,
        rank_decay: float = CONFIDENCE_RANK_DECAY,
) -> float:
    """
    Aggregate per-line scores into a single image-level confidence value in [0, 1].
    The raw sum is passed through ``1 - exp(-steepness × sum)`` to map it into [0, 1]
    with exponential growth — low raw sums stay near 0, high sums saturate toward 1.
    No lines detected → confidence 0.0 exactly.

    Args:
        lines:       list of line dicts returned by detect_line_artefact / _detect_axis.
        width_ref:   reference width in pixels (contribution multiplier = 1.0 at this width).
        width_power: exponent applied to the normalised width.
        steepness:   controls how quickly the exponential curve rises.
        rank_decay:  geometric decay factor per rank (0 < rank_decay < 1).
    ...
    # Compute per-line weight: peak_score × (width / width_ref)^width_power
    weights = [
        ln['peak_score'] * (ln['width_px'] / width_ref) ** width_power
        for ln in lines
    ]

    # Sort descending so the strongest line is always rank 0
    weights.sort(reverse=True)

    # Rank-decayed sum
    raw = sum(w * (rank_decay ** i) for i, w in enumerate(weights))

    # Map to [0, 1] via exponential saturation
    return float(1.0 - np.exp(-steepness * raw))


def detect_line_artefact(
        img_arr: NDArray,
        ...
        confidence_width_ref: float = CONFIDENCE_WIDTH_REF,
        confidence_width_power: float = CONFIDENCE_WIDTH_POWER,
        confidence_steepness: float = CONFIDENCE_STEEPNESS,
        confidence_rank_decay: float = CONFIDENCE_RANK_DECAY,
) -> tuple[list[dict],float]:
    """
    Detect image artefact lines in an image and return line descriptors plus an image-level
    confidence score.
    ...
    Args:
        ...
        confidence_width_ref:    reference width for the confidence formula (px)
        confidence_width_power:  exponent applied to normalised width
        confidence_steepness:    exponential steepness for confidence curve
        confidence_rank_decay:   geometric rank-decay factor for confidence formula

    Returns:
        Tuple of:
          - all_lines: list of dicts describing every detected image artefact line
          - confidence: float in [0, 1] representing how certain we are the
            image contains an image artefact anomaly
    """

    gray = _load_gray_from_array(img_arr)
    ...
    confidence = compute_confidence(
        all_lines,
        width_ref=confidence_width_ref,
        width_power=confidence_width_power,
        steepness=confidence_steepness,
        rank_decay=confidence_rank_decay,
    )

    return all_lines, round(float(confidence), 5)